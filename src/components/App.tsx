...
function App() {
  ...
  const nextLetter = useAppSelector(
    (state) => state.nextLetter.nextLetterSlice
  );
  const guessesRemaining = useAppSelector(
    (state) => state.guessesRemaining.guessesRemainingSlice
  );
  const currentGuess = useAppSelector(
    (state) => state.currentGuess.currentGuessSlice
  );
  const { currentWord } = useAppSelector(
    (state) => state.rightGuess.rightGuessSlice
  );
  const { open, window, title } = useAppSelector(
  ...
  function checkGuess() {
    ...
    const defeat = () => {
      ...
      dispatch(
        activeModal({ open: true, window: "GameLost", title: "Проиграл" })
      );
    };
    const nextStep = () => {
    ...
    if (active) {
      console.log("on");
      if (letters.length === 0 || (letters.length > 0 && letters.every((item) => currentGuess.includes(item)))) {
